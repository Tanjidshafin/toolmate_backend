...
module.exports = ({ storeLocationStorage, auditLogger, getUserInfoFromRequest, toolAnalyticsStorage }) => {
  ...
  router.get("/store-locations/closest", async (req, res) => {
    try {
      const { lat, lon, limit = 3, suburb, postcode, retailer } = req.query
      if (!lat || !lon) {
      ...
      if (isNaN(userLat) || isNaN(userLon) || Math.abs(userLat) > 90 || Math.abs(userLon) > 180) {
      ...
      if (retailer) {
        query.retailer = { $regex: retailer, $options: "i" }
      }
      const pipeline = [
        { $match: query },
        {
      ...
      const closestStores = await storeLocationStorage.aggregate(pipeline).toArray()

      await toolAnalyticsStorage.insertOne({
        type: "closest_stores_search",
        timestamp: new Date(),
        userLocation: { lat: userLat, lon: userLon },
        filters: { suburb, postcode, retailer },
        resultCount: closestStores.length,
        limit: limitNum,
      })

      res.json(closestStores)
    } catch (error) {
      console.error("Error fetching closest store locations:", error)
      res.status(500).json({ error: "Failed to fetch closest store locations" })
    }
  })

  router.get("/store-locations/closest-by-location", async (req, res) => {
    try {
      const { suburb, postcode, limit = 3, retailer } = req.query

      if (!suburb && !postcode) {
        return res.status(400).json({ error: "Either suburb or postcode is required" })
      }
      ...
      const stores = await storeLocationStorage.find(query).limit(limitNum).sort({ store_name: 1 }).toArray()

      await toolAnalyticsStorage.insertOne({
        type: "location_based_stores_search",
        timestamp: new Date(),
        searchCriteria: { suburb, postcode, retailer },
      ...
      res.status(500).json({ error: "Failed to fetch stores by location" })
    }
  })
